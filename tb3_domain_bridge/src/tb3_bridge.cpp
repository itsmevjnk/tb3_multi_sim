...
int main(int argc, char **argv) {
    ...
    auto tb_nspace = nspace.value();

    std::cout << "Bridging Turtlebot topics and services (on namespace " << tb_nspace << ") from domain " << from_domain_id << " to " << to_domain_id << std::endl;

    /* set up bridge config */
    domain_bridge::DomainBridgeConfig config;
    std::string topic_prefix = "/" + tb_nspace;
    ...
    config.topics = {
        ...
        {domain_bridge::TopicBridge{topic_prefix + "/robot_description", "std_msgs/msg/String", from_domain_id, to_domain_id}, domain_bridge::TopicBridgeOptions().remap_name("/robot_description")}, // for completeness, not actually needed
